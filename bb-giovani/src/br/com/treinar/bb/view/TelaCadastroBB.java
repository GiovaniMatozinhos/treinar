...
public class TelaCadastroBB {
	...
	public void criarContaGenerica(Conta conta) {
		...
		// Não segue adiante enquanto digitar um número de conta válido!
		int contaExiste = verificarContaExiste(numeroConta);
		while (contaExiste != -1) {
			System.out.print("Erro: O número da conta " + numeroConta + " existe. Digite outro número de conta!\n");
			System.out.print("Informe o novo número da conta: ");
			numeroConta = input.nextInt();
			contaExiste = verificarContaExiste(numeroConta);
		}
	...
	private void pesquisarContaUnica() {
		System.out.println("Digite a numero da conta para pesquisa: ");
		listarContas();
		System.out.print("\n=> ");
		int numeroConta = input.nextInt();
		int posicao = controller.pesquisarConta(numeroConta);
		listarContaPosicao(posicao);
	}

	private void listarContaPosicao(int posicao) {
		if (posicao != -1) {
			Conta[] contas = controller.recuperarContas();
			System.out.println(posicao + " - " + contas[posicao]);
		} else {
	...
	private int verificarContaExiste(int numeroConta) {
		Conta[] contas = controller.recuperarContas();
		int flag = -1;
		for (int i = 0; i < contas.length; i++) {
			if (contas[i] != null) {
